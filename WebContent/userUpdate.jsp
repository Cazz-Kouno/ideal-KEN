...
<jsp:useBean id="userInfo" class="model.User" scope="session"/>
    <form name="userUpdateForm" action="/ideal/controller/UserOperationSvl" method="post" onsubmit="return dataCheck(this);">
        <table>
            ...
            <tr>
                <th>お名前	*</th>
                <td><input type="text" name="usrName" size="20" maxlength="20" style="ime-mode: active" placeholder=<%= userInfo.getUsrName() %> ;/><td>
            </tr>
           <tr>
                <th>住所</th>
                <td><input type="text" name="address" rows="4" cols="50" style="ime-mode: active;" placeholder=<%= userInfo.getAddress()  %> ;/><td>
            </tr>
            <tr>
                <th>電話番号 *</th>
                <td><input type="text" name="phone" size="15" maxlength="20" style="ime-mode: inactive;" placeholder=<%= userInfo.getPhone() %> ;/></td>
            </tr>
            <tr>
                <th>メールアドレス	*</th>
                <td><input type="text" name="mail" size="30" maxlength="100" style="ime-mode: inactive;" placeholder=<%= userInfo.getMail() %> ;/></td>
            </tr>
            <tr>
                <th>パスワード</th>
                <td><input type="password" name="password" size="8" maxlength="8" style="ime-mode: inactive;" placeholder=<%= userInfo.getPassword() %> />※変更時のみ入力してください。</td>
            </tr>
            <tr>            	
                <td id="sub" colspan="2">
                </br>
					<td2>*は必須入力です。</td2><input type="submit" value="変更" />
					 <input type="hidden" name="mode" value=<%= UserOperationSvl.UPDATE %> />
       				 <input type="hidden" name="usrId" size="8" maxlength="8" style="ime-mode: inactive;" />	                   
            	</td>
            </tr>
        </table>    
    </form>
